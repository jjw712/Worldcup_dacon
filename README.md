...
**[게임 바로 플레이](https://jammulma-worldcup.andyjjw712.workers.dev)** · [개발 환경·실행 방법](#로컬-실행)

**개인 프로젝트**로 기획, 경기 시뮬레이션 엔진, UI, 테스트와 배포를 담당했습니다.  
기술: TypeScript · React · Cloudflare Workers · Vitest

## 게임 화면

### 처음 화면 · 캠페인 시작

다음 상대의 특징과 조별리그 순위를 확인하고 작전실로 이동합니다.

![캠페인 시작 화면](docs/images/campaign-start.png)
...
- 경기 중지 화면과 경기 후 리포트에서 캠페인 처음부터 다시 시작
...
2. 경기 흐름을 관찰하고, 중지하면 즉시 선수 교체 또는 교체 예약을 합니다. 경기 시작 후 전술 변경은 하이드레이션·하프타임에만 가능합니다.
3. 하이드레이션 브레이크에는 지시를 선택해 전달합니다. 서브 전술은 선택 후 `적용`을 눌러야 전환되고 시간이 차감됩니다.
...
| 선발·교체 변경 | 선수 선택 후 다른 선수 명단에 마우스를 올려 교체 버튼 클릭, 또는 드래그 |
...
하프타임 개인 체력 안배는 여러 선수를 선택할 수 있으며, 선수당 1 AP가 듭니다.
같은 선수를 다시 선택하면 취소됩니다. 캠페인 재시작은 확인 후 현재 진행을 초기화합니다.
...
직접 배포하려면 본인 Cloudflare 계정의 Wrangler 인증이 필요합니다.
...
- Cloudflare Workers
- HTML Canvas 2D 전술판
- Vitest 단위 테스트, Node.js 서버 렌더링 테스트
- GitHub Actions CI (타입 검사·린트·테스트)
